...
syn region   puppetDefine            start="\(\<class\>\|\<define\>\|\<application\>\|\<type\>\|\<function\>\|\<plan\>\)\s\{-}[a-z0-9_-]\+\(::[A-Za-z0-9_-]\+\)*" end="{"me=s-1 nextgroup=puppetDefineContent contains=puppetDefType,puppetDefArguments,puppetClassInherit
syn match    puppetDefType           "\(\<class\>\|\<define\>\|\<node\>\|\<application\>\|\<type\>\|\<function\>\|\<plan\>\)" contained
...
syn region   puppetDefDataType       start="\<type\>" end="{"me=s-1 contains=puppetDefType,puppetString,puppetDefArguments nextgroup=puppetDataTypeContent skipwhite skipempty extend
syn region   puppetDataTypeContent   matchgroup=puppetBraces start=/{/  end=/}/ contains=ALLBUT,puppetDefDataType transparent fold contained keepend
...
syn keyword  puppetKeyword           import inherits include require contain produces consumes
